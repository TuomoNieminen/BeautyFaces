...
I combined the pictures with the data provided by Sutela to form the competition data, which then included the demographic information, body measurements, past competition results, as well as numeric vectors representing greyscale pictures of the competitors faces. The data also included information on the competitors of the Miss Finland 2016 (not yet held at the time). For the 2016, results were of course missing.
...
- I chose to use *regularisation* to further reduce the number of predictors in the regression model. Regularisation essentially forces some of the weights of the predictors to zero, removing their effect from the model. This can help to fight overfitting.
...
The helper functions used in the analysis can be found in the [GitHub repository](https://github.com/TuomoNieminen/BeautyFaces/blob/master/missR.R) related to this page. The analysis also uses the R packages LiblineaR, IM and dplyr.  
...
```{r competitors2016, fig.width = 9, fig.height = 9, fig.cap = "The 2016 Miss Finland competitors"}
...
```{r histeq, fig.width = 9, fig.height = 9, fig.cap = "A sample of faces after histogram equalisation"}
...
```{r, binarize, fig.width = 9, fig.height = 9, fig.cap = "A sample of binarized Miss Finland competitor faces"}
...
That's pretty artsy. We could also take a look at some averages again. Here is the average of all the histogram equalised faces.
...
When we have the principal components solution, we can use it as a projection to create a lower dimension representation of any image. We can also do another projection to represent the picture in it's original dimensions but with the approximate principal component solution. These are sometimes called eigenfaces, because the PCA representation is given by *eigenvectors* of the original data matrix.
...
```{r, fig.width = 8, fig.height = 8, fig.cap = "Comparison of two original pictures and eigenface representations using the first 86 principal components"}
...
```{r, fig.width = 8, fig.height = 8, fig.cap = "Comparison of two original pictures and eigenface representations using the first 50 principal components"}
...
```{r, fig.width = 8, fig.height = 8, fig.cap = "Comparison of two original pictures and eigenface representations using the first 7 principal components"}
...
To accomplish this I used l1 regularized logistic regression, using different values for the cost parameter as well as different eigenface representations to make predictions, and then studied which choices yielded the best results. To select the optimal parameters, I chose a cross-validation method were the cross-validation was performed by year: each round of cross-validation a single competition year was set aside as the testing group. The model was then fit with rest of the data and tested on the test year data. 

I defined the gain function to be the accuracy of correctly predicting the top three competitors for each year. I gave the model some slack and compared the top 4 predictions of the model to the actual top 3.  
...
The table below shows which cost and eigenface dimensionality (facedim) were in the top 20% performing models, when measured by their accuracy in the cross-validation rounds.  
...
After finding the optional cost and facedim parameters, I  predicted the 2016 results using again the same l1 regularized logistic regression technique with each of the best cost and facedim combinations separately. The hard lifting is done by the  `LiblineaR::LiblineaR` function for which I wrote a wrapper. The result of the wrapper is list with two components: a matrix of weights and a matrix of probabilities. 
...
The basic goal in regression analysis is to find weights to describe the relationship between the explanatory features (variables) and the target feature. The following table shows the distribution of weights for each feature in the data, learned using l1 reguralized logistic regression. 


```{r}
sdf <- do.call(cbind, lapply(data.frame(res$weights), summary)) %>% t %>% data.frame
sdf$feature <- rownames(sdf)
sdf
```

The negative value of the AsuinP feature tells us that the winners are most often from the southern part of Finland compared to the middle or northern parts. However we can also see that the ones who get to the top 3 are not usually from Helsinki and you might be better off if you hail from Turku instead. All of the 'curviness' features vyotaro (waist), rinta ( chest), lantio (hips) as well as pituus (height) have on average small positive weights associated to them. So all things equal you want to be a little curvy and a bit taller than the competition.  

## Making predictions

Using the computed weights, predictions were made to get a matrix of probabilities, which describe the competitor's changes of reaching the top 3. In the probability matrix, each row represents a competitor and each column is a single predicted probability of getting to the top 3 of the competition, outputted by one of the logistic regression models.

To get the final predictions, I averaged the rows of the probability matrix to get a single probability value for each competitor.
...
```{r faces2016_predictions, fig.width = 9, fig.height = 9, fig.cap = "Predictions for the 2016 Miss Finland competition. The percentages give the probabilities of reaching the top 3. The statistical model predicts Shirly Karvinen and Emilia Seppänen as the favourites."}
