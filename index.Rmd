...
Miss Finland is a beuty contest held annually in Finalnd. During the spring of 2016, as the head of news for the tyyppiarvo.com magazine, I presented an open challenge to forecast the winner of the Miss Finland 2016 competition using statistical analysis. 

Earlier during the spring, the whole editorial staff and other volunteers involved with tyyppiarvo.com had gone through a tremendous effort of collecting and editing pictures of past competotitors to enable to competition. 

**Joni Oksanen** provided a template to make it easier to modify the pictures to be as similar to each other as possible. **Tommi Mäklin** went trough the trouble of finding suitable R functions to transform the pictures to machine-readable numerical vectors, and also provided some helpful plotting functions. We also received help from **Mika Sutela**, a researcher at the University of Eastern Finland, who had access to a dataset of measurements and demograpchic information of past competitors.

I then combined these two data to form a competition dataset, which included both the demographic and past competition results, as well as pictures of the competitors faces. The data also included information on the competitors of the not yet held Mis Finland 2016.
...
This analysis uses the R libraries visible below. The missR functions can be found in the [GitHub repository](https://github.com/TuomoNieminen/BeautyFaces/blob/master/missR.R) related to this page.
...
The columns in the positions 19 to 4114 include the picture vectors. 
...
I did regularized logistic regression using different values for the cost parameter. I chose a modified method were the cross-validation was performed by year: each round one competition yer was set aside as the testing group, the model fit with rest of the data and tested on the test year data. 

I defined the cost function to be the accuracy of correctly predicting the top three competitors for each yer. I compared the best 4 pics of the model to the top 3.  
...
AFter computing the results, we can check which models were the top 20% when measured by their accuracy.  
...
Then, for each of the best cost + face dimension combinations, I  predicted the 2016 results using again the same l1 regularized losigtic regression. I wrote a wrapper function which uses `LiblineaR::LiblineaR` to do the predicting. The result is a probability matrix, where each row represents a competitors and the columns are predicted probabilities of getting to the top 3 of the competition
