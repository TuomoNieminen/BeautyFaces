...
After finding the optional cost and facedim parameters, I  predicted the 2016 results using again the same l1 regularized losigtic regression technique with each of the best cost and facedim combinations separately. The hard lifting is done by the  `LiblineaR::LiblineaR` function which I wrote a wrapped for. The result of the wrapper is list with two components: a matrix of weights and a matrix of probabilities. 
...
The basix goal in regression analysis is to find weights to describe the relationship between the explanatory features (variables) and the target feature. The following table shows the distribution of weights for each feature in the data, learned using l1 reguralized logistic regression. 
...
The negative value of the AsuinP feature tells us that the winners are most often from the southern part of Finland compared to the middle or northern parts. However we can also see that the ones who get to the top3 are not usually from Helsinki and you might be better off if you instead hail from Turku.  All of the 'curviness' features vyotaro (waist), rinta ( chest), lantio (hips) as well as pituus (height) have on average small positive weights associated to them. So all things equal you want to be a little curvy and a bit taller than the competition.  

## Making predictions

Using the computed weights, I then made predictions to get a matrix of probabilities, describing the competitor's changes of reaching the top 3. In the probability matrix, each row represents a competitor and each column is a single predicted probability of getting to the top 3 of the competition, outputted by one of the logistic regression models.
